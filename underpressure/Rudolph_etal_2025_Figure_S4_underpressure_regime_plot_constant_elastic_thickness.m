...
uniaxial_compressive_strength = 40e6;
...
label = sprintf('_initial-%f_strength-%e_mu-%f_const-elastic-%f',initial_frozen_fraction,uniaxial_compressive_strength,muf,elastic_fraction);
...
for i = 1:nf
    ...
    for j=1:nr
        ...
        elastic_thickness = elastic_fraction*(p.R-p.rc);
        z_values = linspace(0,-(p.R-elastic_thickness-p.rc)*(initial_frozen_fraction)+1,nthick);  % amount of thickening (thinning negative)
        ...
        xi_values = p.R - ones(size(z_values))*elastic_thickness;        % coordinate of base of elastic layer
